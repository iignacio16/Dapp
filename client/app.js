App = {
    ...
    loadContract: async () => {
      try {
        ...
        App.contractDeploy = await App.contracts.TasksContract.deployed();
      } catch (error) {
        console.error(error);
      }
    },
    ...
    renderTasks: async () => {
      const tasksCounter = await App.contractDeploy.tasksCounter();
      const taskCounterNumber = tasksCounter.toNumber();
  
      let html = "";
  
      for (let i = 1; i <= taskCounterNumber; i++) {
        const task = await App.contractDeploy.tasks(i);
    ...
    createTask: async (title, description) => {
      try {
        const result = await App.contractDeploy.createTask(title, description, {
          from: App.account,
        });
    ...
    toggleDone: async (element) => {
      ...
      await App.contractDeploy.toggleDone(taskId, {
        from: App.account,
      });
      window.location.reload();
    },
